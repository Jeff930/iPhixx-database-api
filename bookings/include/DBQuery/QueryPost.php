...
class DBQuery
{
	...
	public function getBookings() {
		$sql = "SELECT `bookings`.`bookings_id`,`customers`.`customer_fname`, `customers`.`customer_lname`,`customers`.`location`, `agents`.`agent_lname`,`agents`.`agent_fname`,`agents`.`agent_lname`,`booking_timestamps`.`created_at`, `phone_brands`.`phone_brand` , `device_models`.`model_name`,`lead_statuses`.`lead_status` FROM 
				`bookings` 
							...
							LEFT JOIN  `lead_statuses` ON `bookings`.`leadstatus_no` = `lead_statuses`.`leadstatus_no` WHERE `lead_status` != '3'";
		$result = mysqli_query($this->db,$sql);
		$row=mysqli_fetch_all($result,MYSQLI_ASSOC);
		return $row;
	}
	...
	public function getInvoicesByPage($params){
		...
		$sql_count = "SELECT `invoices`.`invoice_no`,`invoices`.`settled_Timestamp`,`invoices`.`unsettled_Timestamp`, `customers`.`customer_fname`,`customers`.`customer_lname`, `invoice_statuses`.`invoice_status`,`bookings`.`total_price`, `tax`.`tax_value`,`tax`.`tax_name` FROM `bookings` 
				LEFT JOIN `invoices` ON `bookings`.`invoice_no` = `invoices`.`invoice_no` 
				LEFT JOIN `customers` ON `bookings`.`customer_id` = `customers`.`customer_id` 
				LEFT JOIN `invoice_statuses` ON `bookings`.`invoicestatus_no` = `invoice_statuses`.`invoicestatus_no` 
				LEFT JOIN `tax` ON `invoices`.`tax_id` = `tax`.`tax_id` ";
		...
		$sql = "SELECT `invoices`.`invoice_no`,`invoices`.`settled_Timestamp`,`invoices`.`unsettled_Timestamp`, `customers`.`customer_fname`,`customers`.`customer_lname`, `invoice_statuses`.`invoice_status`,`bookings`.`total_price`, `tax`.`tax_value`,`tax`.`tax_name` FROM `bookings` 
				LEFT JOIN `invoices` ON `bookings`.`invoice_no` = `invoices`.`invoice_no` 
				LEFT JOIN `customers` ON `bookings`.`customer_id` = `customers`.`customer_id` 
				LEFT JOIN `invoice_statuses` ON `bookings`.`invoicestatus_no` = `invoice_statuses`.`invoicestatus_no` 
				LEFT JOIN `tax` ON `invoices`.`tax_id` = `tax`.`tax_id` 
	...
	public function getModels($params){
		$sql = "SELECT `devicemodel_id`, `model_name`, `model_number`, `screenrep_price`, `headrep_price`, `earrep_price`, `powerrep_price`, `rearcamrep_price`, `frontcamrep_price`, `homerep_price`, `microphone_price`, `chargeport_price`, `volumerep_price`, `battrep_price`, `signalrep_price`, `backglass_price`,`harddrive_rep`,`hdmirep_price`, `devtype_id`, `devicebrand_id` FROM `device_models` WHERE `devtype_id` = '{$params['devtype_id']}' AND `devicebrand_id`  = '{$params['phonebrand_id']}'";
	...
	public function getAllModelsByPage($params){
		...
		$sql = "SELECT `devicemodel_id`, `model_name`, `model_number`, `screenrep_price`, `headrep_price`, `earrep_price`, `powerrep_price`, `rearcamrep_price`, `frontcamrep_price`, `homerep_price`, `microphone_price`, `chargeport_price`, `volumerep_price`, `battrep_price`, `signalrep_price`, `backglass_price`,`harddrive_rep`,`hdmirep_price`, `devtype_id`, `devicebrand_id`,`active` FROM `device_models` WHERE active = '1' LIMIT 15 OFFSET {$offset}";
	...
	public function getAllModels(){
		$sql = "SELECT `devicemodel_id`, `model_name`, `model_number`, `screenrep_price`, `headrep_price`, `earrep_price`, `powerrep_price`, `rearcamrep_price`, `frontcamrep_price`, `homerep_price`, `microphone_price`, `chargeport_price`, `volumerep_price`, `battrep_price`, `signalrep_price`, `backglass_price`,`harddrive_rep`,`hdmirep_price`, `devtype_id`, `devicebrand_id`,`active` FROM `device_models`";
	...
	public function addModel($body){
		...
		$sql = "INSERT INTO `device_models`(`devicemodel_id`, `model_name`, `model_number`,`active`, `screenrep_price`, `headrep_price`, `earrep_price`, `powerrep_price`, `rearcamrep_price`, `frontcamrep_price`, `homerep_price`, `microphone_price`, `chargeport_price`, `volumerep_price`, `battrep_price`, `signalrep_price`, `backglass_price`, `trackpad_price`, `hdmirep_price`, `harddrive_rep`, `devtype_id`, `devicebrand_id`,`active`) VALUES ( NULL,'{$body['model_name']}','{$body['model_number']}','1',"; 
		...
		if ($body['frontcamrep_price'] != 'null'){
			$sql = $sql." '{$body['frontcamrep_price']}',";
		}
		else{
			$sql = $sql." NULL,";
		}
		...
		if ($body['harddrive_rep'] != 'null'){
			$sql = $sql." '{$body['harddrive_rep']}',";
		}
		else{
			$sql = $sql." NULL, ";
		}

		$sql = $sql." '{$body['devtype_id']}','{$body['devicebrand_id']}',1)";
