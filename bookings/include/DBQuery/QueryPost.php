...
class DBQuery
{
	...
	public function addCustomer($body){
		$sql = "INSERT INTO `customers` (`customer_id`, `customer_fname`, `customer_lname` , `email`, `phone`,`altPhone`, `birthdate`,`address`) VALUES (NULL, '{$body["firstName"]}', '{$body["lastName"]}' ,  '{$body["email"]}', '{$body["mobile"]}','{$body["phone"]}', '{$body["birthdate"]}','{$body["address"]}')";
		$result = mysqli_query($this->db, $sql);
		if ($result) {
